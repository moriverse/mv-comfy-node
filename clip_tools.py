...
class CLIP_Interrogator:
    ...
    def INPUT_TYPES(s):
        return {
            "required": {
                ...
                "save_prompt_to_txt_file": (
                    "STRING",
                    {"default": "clip_interrogator_prompt.txt"},
                ),
            }
        }
    ...
    def interrogate(
        ...
        save_prompt_to_txt_file=None,
    ):

        self.keep_model_alive = keep_model_alive

        print(
            f"Interrogating image with mode {mode}, keep_model_alive={keep_model_alive}"
        )

        # ci expects a PIL image, but we get a torch tensor:
        if image.shape[0] > 1:
            print(
                "Warning: CLIP_Interrogator expects a single image, but got a batch. Using first image in batch."
            )
            ...
            if global_interrogator_model:
                ...
                self.ci = Interrogator(
                    Config(
                        ...
                        cache_path=os.path.join(BLIP_MODEL_DIR, "cache"),
                    )
                )
