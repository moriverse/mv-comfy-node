...
class Interrogator:
    ...
    def interrogate_classic(
        self, image: Image, max_flavors: int = 3, caption: Optional[str] = None
    ) -> str:
    ...
    def interrogate_fast(
        self, image: Image, max_flavors: int = 32, caption: Optional[str] = None
    ) -> str:
    ...
    def _load_cached(self, desc: str, hash: str, sanitized_name: str) -> bool:
        ...
        if self.config.download_cache and not os.path.exists(cached_safetensors):
            ...
            try:
                os.makedirs(self.config.cache_path, exist_ok=True)
                _download_file(
                    download_url, cached_safetensors, quiet=self.config.quiet
                )
        ...
        if os.path.exists(cached_safetensors):
            ...
            if "hash" in tensors and "embeds" in tensors:
                if np.array_equal(
                    tensors["hash"], np.array([ord(c) for c in hash], dtype=np.int8)
                ):
...
def _download_file(
    url: str, filepath: str, chunk_size: int = 4 * 1024 * 1024, quiet: bool = False
):
